"""Structured Chat handler — forwards OpenAI-format messages + tools to the LLM.

Handles benchmarks that send structured DataParts with conversation history
and tool schemas (e.g. Pi-Bench policy compliance evaluation). This is a
general chat-completion proxy, not benchmark-specific logic.
...
logger = logging.getLogger("agentwhetters.structured_chat")
...
class StructuredChatHandler:
    ...
    def is_structured_chat(self, data: dict) -> bool:
        """Detect if a DataPart payload is a structured chat request."""
        return (
    ...
    async def handle_turn(self, data: dict) -> dict:
        ...
        tracker.record(response, label="structured_chat")
